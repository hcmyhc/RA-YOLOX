...
class Align_weight(nn.Module):

    def __init__(self, channel_1, channel_2):
        super().__init__()
        self.down = nn.Conv2d(channel_1, channel_1//2, 1)
        self.tot_1 = nn.Conv2d(channel_1//2, channel_2, 1)
        self.tot_2 = nn.Conv2d(channel_1//2, channel_2, 1)
        self.sig = nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        x = self.down(x)
        x = F.selu(x)
        return self.sig(self.tot_1(x)), self.sig(self.tot_2(x))
...
class YOLOXHead(nn.Module):
    ...
    def get_losses(
        ...
        # cls_obj = cls_preds.view(-1, self.num_classes)
        # cls_weight = (p_pos_weights.unsqueeze(-1).repeat(1, self.num_classes) * cls_targets + cls_obj.sigmoid()[fg_masks].pow(2) * (1.0 - cls_targets)).to(dtype)
        loss_iou = (
                           self.iou_loss(bbox_preds.view(-1, 4)[fg_masks], reg_targets) * p_pos_weights
                   ).sum() / pos_af
        # neg = (cls_obj[fg_masks] * cls_targets).sum(-1)
        # cls_loss_p1 = (self.bcewithlog_loss(cls_obj[fg_masks], cls_targets.to(dtype)) * cls_weight).sum() / pos_af
        # cls_loss_p2 = (self.bcewithlog_loss(neg, torch.zeros_like(neg)) * p_neg_weights).sum() / neg_af
        # obj_preds = obj_preds.view(-1, 1)
        # loss_obj_p1 = (self.bcewithlog_loss(obj_preds[fg_masks], torch.ones_like(obj_preds[fg_masks])
        #                                     ) * p_pos_weights.unsqueeze(-1)).sum() / pos_af
        # loss_obj_p2 = (self.bcewithlog_loss(obj_preds[fg_masks], torch.zeros_like(obj_preds[fg_masks])
        #                                     ) * p_neg_weights.unsqueeze(-1)).sum() / neg_af
        # loss_obj_p3 = (self.bcewithlog_loss(obj_preds[~fg_masks], torch.zeros_like(obj_preds[~fg_masks])
        #                                     ) * obj_preds[~fg_masks].sigmoid().pow(2)).sum() / num_fg
        # loss_cls = cls_loss_p1 + cls_loss_p2
        # loss_obj = loss_obj_p1 + loss_obj_p2 + loss_obj_p3
    ...
    def get_assignments(
        ...
        (
            ...
            pos,
            neg
        ) = self.dynamic_k_matching(p_pos, pair_wise_ious, gt_classes, num_gt, fg_mask, p_cls, is_in_boxes_and_center)
    ...
    def dynamic_k_matching(self, cost, pair_wise_ious, gt_classes, num_gt, fg_mask, p_cls, is_in_boxes_and_center):
        # Dynamic K
        # ---------------------------------------------------------------
