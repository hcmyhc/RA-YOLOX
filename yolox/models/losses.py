...
class IOUloss_xxyy(nn.Module):
    def __init__(self, reduction="none", loss_type="iou"):
        super(IOUloss_xxyy, self).__init__()
        self.reduction = reduction
        self.loss_type = loss_type
...
class Focal_loss(nn.Module):
    ...
    def forward(self, preds, labels):
        ...
        preds_neg = preds[~mask]
        preds_pos_loss = (1-preds_pos)**self.gamma*F.binary_cross_entropy(preds_pos, torch.ones_like(preds_pos))
        preds_neg_loss = (preds_neg)**self.gamma*F.binary_cross_entropy(preds_neg, torch.zeros_like(preds_neg))
        loss = torch.cat(((self.alpha)*preds_pos_loss, (1-self.alpha)*preds_neg_loss), dim=-1)
        if self.reduction == "mean":
            loss = loss.mean()
        elif self.reduction == "sum":
            loss = loss.sum()
